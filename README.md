...
1. **Target our marketing based on time and location.** If we are able to target our marketing to casual users based on time, I would recommend targeting them May through mid-October, 7 days a week.  If we can target time of day, I would recommend 7:00am through about 4:00pm. If we are able to target our marketing based on location, I recommend targeting hotels, attractions and popular places along or proximate to the waterfront.
2. **A/B Test two different marketing messages.** Annual members and casual riders use Cyclisting for completely different purposes. I recommend running A/B tests to see if it is more effective to market to casual riders: a) with the goal of creating repeat usage for their purpose, b) with the goal of converting them to commuters.
3. **Acquire deeper data to achieve better targeting.** If we could obtain and analyze more data, such as associating riders with rides and obtaining rider zip codes, we could develop insights that would enable us to deploy much more targeted marketing to much more specific segments.
